...
struct Students
{
    ...
    int grade_math;
    int grade_info;
    Students() : surname(""), course(0), speciality(IT), grade_physics(0), grade_math(0), grade_info(0) {}
};
...
bool b_search(Students* S, const int N, const string surname, const int course, const double average_evaluation) {
    ...
    while (left <= right) {
        ...
        cout << "Перевірка: " << S[mid].surname << ", курс: " << S[mid].course
            << ", середній бал: " << student_average_evaluation(S, mid) << endl;

        if (S[mid].surname < surname ||
            (S[mid].surname == surname && S[mid].course < course) ||
            (S[mid].surname == surname && S[mid].course == course && student_average_evaluation(S, mid) < average_evaluation)) {
            left = mid + 1;
        }
            ...
            if (S[mid].surname == surname && S[mid].course == course && areDoublesEqual(student_average_evaluation(S, mid), average_evaluation)) {
                return true;
            }
            break;
        }
